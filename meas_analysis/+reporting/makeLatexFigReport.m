...
for jj = 1:NN
    ...
    for kk = 1:KK
        ...
        elseif ~isempty(strfind(png_fname,'__plvacq'))
            pngs(1).title_str = sprintf('Path Loss versus Acquisition Order (%s)', mat_root_str);
        ...
        elseif ~isempty(strfind(png_fname,'__pl'))
            extra = sprintf('The effective slope for the path loss slope of this measured route is %0.2f dB per decade of distance in meters.', abs(stats.path_gain_dB_poly(1)));

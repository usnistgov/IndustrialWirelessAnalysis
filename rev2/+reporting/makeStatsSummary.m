...
for fk = 1:Nfiles
    ...
    % delete outliers 
    stats.K = deleteoutliers(stats.K);
    stats.mean_delay_sec = deleteoutliers(stats.mean_delay_sec);
    stats.rms_delay_spread_sec = deleteoutliers(stats.rms_delay_spread_sec);
    
    Cstats(fk,:) = {   
        meta.MatFile_str, meta.Frequency_GHz_num, ...
        RxPol, meta.ReceiverAntennaGain_dBi_num, ...
        TxPol, meta.TransmitterAntennaGain_dBi_num, ...
        -(stats.path_gain_dB_poly(1))/10, stats.path_gain_dB_poly(2), ...

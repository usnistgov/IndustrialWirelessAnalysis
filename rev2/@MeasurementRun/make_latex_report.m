function make_latex_report(fname, ~)
...
if nargin < 1
    fname = 'D:\FACTORY_RF_MEASUREMENTS\FACTORY_MEASUREMENTS4_UPD2023\active_meas\pub\test_report.tex';
end
...
%f = nan;
%if exist(fname,'file')
    [f,errMsg] = fopen(fname,'w');  % standard output
    if f == -1 
        error(errMsg)
    end
%end
    ...
    for jj = 1:NN
        ...
        for kk = 1:length(pngs)
            if ~isempty(pngs(kk).png_fname)
                ...
                copyfile(png_path, ['pub/figs2/' png_file2]);
